...
class TimetableOptimizer:
    ...
    def calculate_subject_blocks(self, subject):
        ...
        if preference == 'single':
            # All classes are single 45-minute slots
            return [1] * hours_per_week
        elif preference == 'continuous':
            # All classes are in continuous blocks
            blocks = []
            remaining = hours_per_week
            ...
            # Mixed approach: prefer continuous blocks but use singles for remainder
            blocks = []
            remaining = hours_per_week
            
            # Add continuous blocks first
            while remaining >= block_size:
                blocks.append(block_size)
                remaining -= block_size
            
            # Add remaining as single classes
    ...
    def can_schedule_block(self, day_idx, start_time_slot, faculty_id, classroom_id, existing_schedule, block_size):
        ...
        # Special handling for lab sessions (3-hour blocks)
        if block_size >= 3 and start_time_slot in self.get_lab_time_slots():
            # For lab sessions, check if the entire lab slot is available
            return self.is_slot_available(day_idx, start_time_slot, faculty_id, classroom_id, existing_schedule)
        
        # Get consecutive time slots for the block
    ...
    def generate_single_timetable(self, batch_id, semester):
        ...
        for subject in required_classes:
            ...
            while not scheduled and attempts < max_attempts:
                ...
                if subject.get('requires_lab', False) and block_size >= 3:
                    # For lab sessions, use specific lab time slots
                    lab_slots = self.get_lab_time_slots()
                    time_slot = random.choice(lab_slots)
                else:
                ...
                if self.can_schedule_block(day_idx, time_slot, faculty['id'], classroom['id'], schedule, block_size):
                    ...
                    if (daily_hours[day_idx] + block_size <= faculty.get('max_hours_per_day', 6) and 
                        ...
                        if batch_daily_classes + block_size <= self.max_classes_per_day:
                            # Special handling for lab sessions (3-hour blocks)
                            if subject.get('requires_lab', False) and block_size >= 3 and time_slot in self.get_lab_time_slots():
                                # Validate all required IDs are present
                                if not all([subject.get('id'), faculty.get('id'), classroom.get('id'), batch_id]):
                                    print(f"ERROR: Missing required IDs for lab session - subject_id: {subject.get('id')}, faculty_id: {faculty.get('id')}, classroom_id: {classroom.get('id')}, batch_id: {batch_id}")
                                    attempts += 1
                                    continue
                                    
                                # For lab sessions, schedule as a single 3-hour block
                                schedule.append({
                                    'day_of_week': day_idx,
                                    'time_slot': time_slot,  # Use the lab time slot directly
                                    ...
                                    'is_continuous_block': True,
                                    'is_lab': True
                                })
                                scheduled = True
                                scheduled_count += 1  # Count as 1 entry for lab
                            else:
                                # Validate all required IDs are present
                                if not all([subject.get('id'), faculty.get('id'), classroom.get('id'), batch_id]):
                                    print(f"ERROR: Missing required IDs for regular class - subject_id: {subject.get('id')}, faculty_id: {faculty.get('id')}, classroom_id: {classroom.get('id')}, batch_id: {batch_id}")
                                    attempts += 1
                                    continue
                                    
                                # Schedule the entire block for regular classes
                                ...
                                for slot in block_slots:
                                    schedule.append({
                                        ...
                                        'is_fixed': False,
                                        'block_size': block_size,
                                        'is_continuous_block': block_size > 1,
                                        'is_lab': False
                                    })
                                scheduled = True
    ...
    def get_lab_time_slots(self):
        """Get specific time slots for lab sessions: 9:00-12:00 and 1:30-4:30"""
        lab_slots = [
            "09:00-12:00",  # Morning lab session (3 hours)
            "13:30-16:30"   # Afternoon lab session (3 hours)
        ]
        return lab_slots
